...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "GQA — Água cristalina. Expertise de quem entende de química." },
      { property: "og:description", content: "Diagnóstico em 10 min · Curadoria de engenheiro químico · Frete grátis." },
...
function VideoHero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <img src={heroPool} alt="" className="video-cover" style={{ transform: `scale(${scale})`, opacity: opacity * 0.6, zIndex: -1 }} aria-hidden />
      <div
      ...
      />
      <div className="relative z-10 container-prose text-center pt-28 pb-32" style={{ opacity: contentOpacity, transform: `translateY(${contentY}px)`, transition: "opacity 0.1s linear" }}>
        <Reveal>
        ...
        <Reveal delay={1}>
          <h1 className="h-hero mt-8 text-white">
            Água cristalina.<br />Expertise de quem<br />entende de química.
          </h1>
        </Reveal>
        <Reveal delay={2}>
          <p className="mt-8 max-w-2xl mx-auto text-lg leading-relaxed" style={{ color: "rgba(255,255,255,0.82)" }}>
            30 anos de engenharia química aplicados à sua piscina. Diagnóstico preciso, produtos certificados e atendimento que resolve de verdade.
          </p>
        </Reveal>
        <Reveal delay={3}>
          <div className="mt-10 flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-4">
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-premium inline-flex items-center justify-center gap-3 px-7 py-4 text-base text-white w-full sm:w-auto" style={{ background: "var(--whatsapp)" }}>
              <WAIcon className="w-5 h-5" /> Diagnóstico Grátis no WhatsApp
            </a>
            <a href="#produtos" className="btn-premium inline-flex items-center justify-center px-7 py-4 border border-white/60 text-base text-white hover:bg-white hover:!text-[#062A40] w-full sm:w-auto">
              Ver Produtos
            </a>
          </div>
...
function AnimOnView({
  ...
  return (
    <div ref={ref} className={`anim-on-view ${direction} ${shown ? "in" : ""} ${className}`} style={{ animationDelay: `${delay}ms` }}>
      {children}
    </div>
  );
}
...
/* ============================================================
   MÉTRICAS — entrada direcional + números corretos
============================================================ */
function Metrics() {
  const items: { label: string; render: ReactNode; dir: "from-left" | "from-bottom" | "scale-center" | "from-right"; delay: number }[] = [
    { label: "Expertise em química",   render: <><span className="text-3xl align-top mr-1">+</span><Counter to={30} /></>, dir: "from-left",     delay: 0 },
    { label: "Clientes atendidos",     render: <><span className="text-3xl align-top mr-1">+</span><Counter to={1000} /></>, dir: "from-bottom",   delay: 150 },
    { label: "Serviços especializados",render: <Counter to={12} />,                                       dir: "scale-center",  delay: 300 },
    { label: "Tempo de diagnóstico",   render: <><Counter to={10} /><span className="text-3xl ml-1">min</span></>, dir: "from-bottom", delay: 450 },
  ...
  return (
    <section className="section-y" style={{ background: "var(--bg-dark)" }}>
      <div className="container-prose grid grid-cols-2 md:grid-cols-5 gap-y-10">
        {items.map((m, i) => (
          <AnimOnView key={i} direction={m.dir} delay={m.delay} className={`text-center px-4 ${i > 0 ? "md:border-l md:border-white/15" : ""}`}>
            <div className="font-title text-5xl md:text-6xl text-white leading-none">{m.render}</div>
...
function Differentials() {
  const items = [
    { icon: <Beaker />, title: "Engenharia química aplicada", text: "30 anos de expertise técnica em tratamento de águas, garantindo dosagens precisas." },
    { icon: <CheckBadge />, title: "Produtos certificados", text: "Marcas reconhecidas e formulações testadas para resultado consistente." },
    { icon: <Store />, title: "Linha completa", text: "Químicos, bombas, filtros, LED, sauna, spa — tudo em um único lugar." },
    { icon: <Truck />, title: "Entrega rápida", text: "Atendemos toda a região com pontualidade. Frete grátis em pedidos." },
    { icon: <Heart />, title: "Consultoria individualizada", text: "Cada cliente recebe um diagnóstico técnico próprio, não receita pronta." },
    { icon: <Gift />, title: "Transparência", text: "Indicação honesta. Recomendamos só o que sua piscina realmente precisa." },
  ];
  return (
          ...
          {items.map((it, i) => (
            <AnimOnView key={i} direction="from-bottom" delay={i * 120}>
              <div className="card-light h-full p-8">
                <div className="text-accent w-12 h-12 flex items-center justify-center rounded-xl" style={{ background: "rgba(0,150,199,0.08)" }}>{it.icon}</div>
...
function VideoBreak() {
  ...
  useEffect(() => {
    ...
    if (window.innerWidth < 1024) { setOffset(0); return; }
    const el = ref.current;
    if (!el) return;
    ...
    <section ref={ref} className="relative overflow-hidden" style={{ height: "70vh" }}>
      ...
      <div className="relative z-10 h-full flex items-center justify-center px-6">
        <div className="text-center max-w-3xl">
          ...
          <Reveal delay={2}>
            <a href="#produtos" className="btn-premium inline-flex items-center justify-center mt-8 px-8 py-4 bg-white !text-[#062A40] font-medium">
...
function ProductCard({ p }: { p: Product }) {
  ...
  return (
    <div className="card-light p-6 flex flex-col" style={{ animation: "overlay-fade 0.4s ease both" }}>
      <div className="aspect-[4/3] rounded-xl mb-5 flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(0,150,199,0.08), rgba(72,202,228,0.15))" }} aria-label={`Placeholder ${p.name}`}>
      ...
      <div className="mt-5 flex items-center gap-2">
        <button
          onClick={() => { add(p.name); setOpen(true); }}
          className="btn-premium flex-1 inline-flex items-center justify-center gap-2 px-3 py-2.5 text-sm text-white"
          style={{ background: "var(--accent)" }}
        >
          Adicionar ao carrinho
        </button>
        <a href={waProduct(p.name)} target="_blank" rel="noopener noreferrer" aria-label={`Pedir ${p.name} no WhatsApp`} className="btn-premium w-10 h-10 inline-flex items-center justify-center text-white shrink-0" style={{ background: "var(--whatsapp)" }}>
...
function ProductsEquipment() {
  ...
  const filtered = useMemo(() => active === "Todos" ? EQUIPMENT : EQUIPMENT.filter((p) => p.cat === active), [active]);
  return (
    <section id="produtos" className="section-y" style={{ background: "#ffffff" }}>
      <div className="container-prose">
        <Reveal className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-6 body-lg">Tudo que sua piscina precisa para funcionar com eficiência e segurança.</p>
        </Reveal>
        <div className="mt-12 filter-scroll md:justify-center">
          {EQ_CATS.map((c) => (
            <button key={c} onClick={() => setActive(c)} className={`filter-chip ${active === c ? "active" : ""}`}>{c}</button>
          ))}
        </div>
...
function ChemistryDivider() {
  return (
    <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #023E8A 0%, #0096C7 100%)" }}>
  ...
  return (
    <div className="chem-card flex flex-col" style={{ borderLeftColor: color }}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] font-semibold uppercase tracking-[0.15em] px-2.5 py-1 rounded-full text-white" style={{ background: color }}>{p.cat}</span>
      ...
      <div className="aspect-[5/3] rounded-lg mb-3 flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${color}10, ${color}25)` }}>
        <div style={{ color }} className="opacity-70">{p.icon}</div>
      </div>
...
function ProductsChemistry() {
  const [active, setActive] = useState("Todos");
  const filtered = useMemo(() => active === "Todos" ? CHEM : CHEM.filter((p) => p.cat === active), [active]);
  return (
    <section id="quimica" className="section-y" style={{ background: "#E8F4FD" }}>
      <div className="container-prose">
        <Reveal className="text-center max-w-3xl mx-auto">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs tracking-[0.15em] uppercase font-medium text-white mb-6" style={{ background: "var(--bg-dark)" }}>
          ...
          <p className="mt-6 body-lg">
            Formulações selecionadas por um engenheiro químico com 30 anos de experiência. Cada produto indicado com precisão técnica.
          </p>
        </Reveal>
        <div className="mt-12 filter-scroll md:justify-center">
          {CHEM_CATS.map((c) => (
        ...
        <div className="mt-12 grid gap-5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))" }}>
          {filtered.map((p, i) => <ChemCard key={`${p.name}-${i}`} p={p} />)}
        </div>
      </div>
    </section>
  );
              ...
              >
                <div className="flip-inner">
                  <div className="flip-face card-dark items-start">
                    ...
                    <p className="mt-auto text-xs uppercase tracking-[0.2em] text-white/50">Toque para detalhes →</p>
                  </div>
                  <div className="flip-face flip-back items-start" style={{ background: "linear-gradient(135deg, #0096C7, #023E8A)", border: "1px solid rgba(255,255,255,0.15)" }}>
    ...
    <section className="section-y" style={{ background: "var(--bg)" }}>
      <div className="container-prose">
        ...
        <div className="mt-20 relative grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-6">
          <div className="hidden md:block absolute top-12 left-[12%] right-[12%] h-px draw-line" style={{ background: "linear-gradient(to right, var(--accent), var(--accent-light), var(--accent))" }} />
          {steps.map((s, i) => (
            <Reveal key={i} delay={((i % 3) + 1) as 1 | 2 | 3}>
              <div className="relative text-center px-4">
                <div className="font-title text-7xl md:text-8xl" style={{ color: "rgba(0,150,199,0.10)" }}>{s.n}</div>
                <div className="relative -mt-12 mx-auto w-16 h-16 rounded-full flex items-center justify-center text-white" style={{ background: "var(--accent)", boxShadow: "0 12px 30px -10px rgba(0,150,199,0.45)" }}>
                  {s.icon}
                </div>
    ...
    <section className="relative overflow-hidden">
      <div className="section-y px-6 text-center" style={{ background: "linear-gradient(135deg, #0096C7 0%, #023E8A 100%)" }}>
        <Reveal>
          ...
          <div className="mt-10">
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-premium inline-flex items-center gap-2 px-8 py-4 bg-white !text-[#023E8A] font-medium">
      ...
      <div className="container-prose grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <Reveal>
          <div className="img-zoom rounded-2xl overflow-hidden relative">
            <img src={aboutPool} alt="GQA — Tratamento técnico de piscinas" loading="lazy" width={1024} height={1024} className="w-full aspect-square object-cover" />
          </div>
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <BadgeBox icon={<Atom />} title="30+ anos" subtitle="Expertise em Engenharia Química" />
            <BadgeBox icon={<Beaker />} title="11 anos" subtitle="no mercado de piscinas" />
          </div>
        </Reveal>
        <Reveal delay={1}>
          ...
          <div className="mt-8 space-y-5 body-lg">
            <p>
              Fundada em 2015, com 11 anos no mercado, nossa empresa nasceu com o propósito de atender indústrias e o mercado de recreação, oferecendo excelência no tratamento de piscinas, unindo conhecimento técnico, produtos de qualidade e atendimento especializado. Atuamos com soluções completas para limpeza, manutenção, tratamento e equilíbrio químico da água, garantindo mais segurança, eficiência e tranquilidade para nossos clientes.
            </p>
            <p>
              Nosso diferencial está no atendimento e expertise técnica: a empresa é liderada por um engenheiro químico com mais de 30 anos de expertise em tratamentos de águas, tanto no segmento de piscinas quanto na área industrial. Esse conhecimento nos permite oferecer diagnósticos precisos, soluções eficientes e uma consultoria individualizada, sempre focada na necessidade de cada cliente.
            </p>
            <p>
              Trabalhamos com compromisso, transparência e alto padrão de atendimento, buscando sempre a melhor solução para manter sua piscina limpa, saudável e pronta para os melhores momentos.
            </p>
            <p className="text-text font-medium">
              Mais do que produtos e serviços, entregamos qualidade e confiabilidade em cada atendimento.
            </p>
          </div>
        </Reveal>
      </div>
...
function BadgeBox({ icon, title, subtitle }: { icon: ReactNode; title: string; subtitle: string }) {
  return (
    <div className="rounded-xl p-5 text-white flex items-center gap-4" style={{ background: "linear-gradient(135deg, #0096C7, #023E8A)" }}>
      <div className="w-11 h-11 rounded-full bg-white/15 flex items-center justify-center shrink-0">{icon}</div>
      <div>
            ...
            return (
              <Reveal key={i} delay={((i % 3) + 1) as 1 | 2 | 3}>
                <div className="card-light overflow-hidden">
                  <button onClick={() => setOpen(isOpen ? null : i)} aria-expanded={isOpen} className="w-full flex items-center justify-between gap-4 p-6 text-left">
                    <span className="font-display text-lg text-text">{f.q}</span>
                    <span className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-accent transition-transform" style={{ background: "rgba(0,150,199,0.10)", transform: isOpen ? "rotate(45deg)" : "rotate(0deg)" }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M5 12h14" /></svg>
                    </span>
                  </button>
                  <div className="grid transition-all duration-400 ease-out" style={{ gridTemplateRows: isOpen ? "1fr" : "0fr" }}>
                    <div className="overflow-hidden"><p className="px-6 pb-6 body-lg">{f.a}</p></div>
                  </div>
                </div>
              </Reveal>
            );
          ...
          <Reveal>
            <div className="space-y-4">
              {cards.map((c, i) => {
                ...
                return (
                  <Wrapper key={i} {...(c.href ? { href: c.href, target: "_blank", rel: "noopener noreferrer" } : {})} className="card-dark flex items-start gap-4 p-6 block hover:!translate-y-[-4px]">
                    <div className="shrink-0 w-11 h-11 rounded-full flex items-center justify-center text-[var(--accent-light)]" style={{ background: "rgba(72,202,228,0.12)" }}>{c.icon}</div>
  ...
  return (
    <footer style={{ background: "#041020" }} className="pt-16 pb-8 px-6">
      <div className="container-prose text-center">
        <img src={logo} alt="GQA" className="logo-halo-footer mx-auto" style={{ height: 56 }} width={160} height={56} />
...
function BeakerSm({ className = "" }: { className?: string }) { return (<svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 3h6M10 3v6L5 20a2 2 0 002 2h10a2 2 0 002-2L14 9V3" /></svg>); }
